...
{
  MOSDRepScrubMap *m = static_cast<MOSDRepScrubMap*>(op->get_req());
...
void PG::handle_scrub_reserve_request(OpRequestRef op)
{
  ...
  if (op->get_req()->get_type() == MSG_OSD_SCRUB_RESERVE) {
    MOSDScrubReserve *m = static_cast<MOSDScrubReserve*>(op->get_req());
    ...
    MOSDSubOp *req = static_cast<MOSDSubOp*>(op->get_req());
...
bool PG::op_must_wait_for_map(epoch_t cur_epoch, OpRequestRef& op)
{
  ...
  case MSG_OSD_PG_BACKFILL_REMOVE:
    return !have_same_or_newer_map(
      cur_epoch,
      static_cast<MOSDPGBackfillRemove*>(op->get_req())->map_epoch);
  ...
  case MSG_OSD_SCRUB_RESERVE:
    return !have_same_or_newer_map(
      cur_epoch,
      static_cast<MOSDScrubReserve*>(op->get_req())->map_epoch);

  case MSG_OSD_REP_SCRUBMAP:
    return !have_same_or_newer_map(
      cur_epoch,
      static_cast<MOSDRepScrubMap*>(op->get_req())->map_epoch);
