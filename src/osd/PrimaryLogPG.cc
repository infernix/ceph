  ...
  )
{
  ...
  if (recovery_info.soid.snap < CEPH_NOSNAP) {
    ...
    if (obc->ssc->snapset.is_legacy()) {
      ...
      auto p = obc->ssc->snapset.clone_snaps.find(hoid.snap);
      assert(p != obc->ssc->snapset.clone_snaps.end());  // hmm, should we warn?
...
inline int PrimaryLogPG::_delete_oid(OpContext *ctx, bool no_whiteout)
{
  ...
  if (get_osdmap()->test_flag(CEPH_OSDMAP_REQUIRE_LUMINOUS)) {
    // in luminous or later, we *cannot* delete the head if there are
    // (or will be) clones; we must set a whiteout instead.
    if (!snapset.clones.empty() ||
	(!ctx->snapc.snaps.empty() && ctx->snapc.snaps[0] > snapset.seq)) {
      dout(20) << __func__ << " have or will have clones; will whiteout" << dendl;
      whiteout = true;
    }
  }
