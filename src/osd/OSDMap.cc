...
static void swap_item(const vector<pair<int32_t,int32_t>>& remapping, int *item)
{
...
void OSDMap::_apply_remap(pg_t pg, vector<int> *raw, int *primary) const
{
  auto p = pg_remap.find(pg);
  if (p != pg_remap.end()) {
    *primary = p->second.first;
    *raw = p->second.second;
    return;
  }
  ...
  if (q != pg_remap_items.end()) {
    swap_item(q->second, primary);
    for (auto& i : *raw) {
      swap_item(q->second, &i);
    }
    return;
  }  
}
  ...
  bool raw_pg_to_pg) const
{
  ...
  if (_acting.empty() || up || up_primary) {
    ...
    _apply_remap(pg, &raw, &_up_primary);
...
void OSDMap::dump(Formatter *f) const
{
  ...
  for (auto& p : pg_remap) {
    ...
    f->dump_stream("pgid") << p.first;
    f->dump_int("primary", p.second.first);
    f->open_array_section("osds");
...
void OSDMap::print(ostream& out) const
{
  ...
  for (auto& p : pg_remap) {
    out << "pg_remap " << p.first << " " << p.second.first
	<< " " << p.second.second << "\n";
  }
  for (auto& p : pg_remap_items) {
