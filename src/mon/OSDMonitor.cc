				      ...
				      map<string,cmd_vartype> &cmdmap)
{
  ...
  if (prefix == "osd setcrushmap" ||
    ...
    int32_t new_primary;
    if (!cmd_getval(g_ceph_context, cmdmap, "primary", new_primary)) {
      ss << "invalid primary osd id value '"
         << cmd_vartype_stringify(cmdmap["primary"]) << "'";
      err = -EINVAL;
      goto reply;
    }
    if (!osdmap.exists(new_primary)) {
      ss << "osd." << new_primary << " does not exist";
      err = -ENOENT;
      goto reply;
    }
    vector<int64_t> id_vec;
    ...
    pending_inc.new_pg_remap[pgid] = make_pair(new_primary, new_pg_temp);
